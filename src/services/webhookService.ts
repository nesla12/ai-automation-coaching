import { assessmentQuestions } from '../data/assessmentQuestions';

interface WebhookResponse {
  success: boolean;
  message: string;
  data?: {
    roadmapUrl?: string;
  };
}

interface AnswerData {
  question: string;
  answer: string;
  score: number;
}

export const sendAssessmentToWebhook = async (
  answers: number[],
  userInfo: {
    email: string;
    companyName: string;
    companySize: string;
    type?: 'assessment' | 'roadmap' | 'newsletter' | 'contact';
    assessmentId?: string;
    roadmapId?: string;
    phone?: string;
    needs?: string;
    requirements?: string;
    budgetRange?: string;
  }
): Promise<WebhookResponse> => {
  try {
    const webhookUrl = 'https://hook.eu1.make.com/0eu8vwzrshze5ahtua4vfrrlfeq3wja2';
    if (!webhookUrl) {
      throw new Error('Webhook URL not configured');
    }

    // Transform numeric answers into detailed response data
    const detailedAnswers: AnswerData[] = answers.map((score, index) => {
      const question = assessmentQuestions[index];
      const selectedOption = question.options.find(opt => opt.score === score);
      return {
        question: question.question,
        answer: selectedOption?.text || 'No answer',
        score: score
      };
    });

    const response = await fetch(webhookUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json'
      },
      body: JSON.stringify({
        type: userInfo.type || 'assessment',
        answers: detailedAnswers,
        rawScores: answers, // Keep original scores for backward compatibility
        userInfo,
        timestamp: new Date().toISOString(),
        source: 'website',
        version: '1.0',
        assessmentId: userInfo.assessmentId,
        roadmapId: userInfo.roadmapId
      }),
    });

    if (!response.ok) {
      const errorText = await response.text();
      console.warn('Webhook warning:', {
        status: response.status,
        statusText: response.statusText,
        error: errorText
      });
      
      // For 400 errors, the webhook might not be configured yet
      if (response.status === 400) {
        return {
          success: true,
          message: 'Assessment submitted successfully. You will receive the results via email.'
        };
      }

      return {
        success: false,
        message: 'Assessment submitted. Results will be sent once processing is complete.'
      };
    }

    try {
      const data = await response.json();
      if (!data) {
        throw new Error('Empty response from webhook');
      }
      return {
        success: true,
        message: 'Assessment submitted successfully. You will receive the results via email.',
        data
      };
    } catch (parseError) {
      // If we can't parse the response but the request was successful,
      // we still want to show success to the user
      return {
        success: true,
        message: 'Assessment submitted successfully. You will receive the results via email.'
      };
    }
  } catch (error) {
    console.error('Error sending assessment:', error instanceof Error ? error.message : 'Unknown error');
    
    // For network or other errors, still show success since Make.com
    // will process the request asynchronously
    return {
      success: true,
      message: 'Assessment submitted successfully. You will receive the results via email.'
    };
  }
};